...
from Extractor.modules.selection_way import selection_way_handler
...
from Extractor.modules.utk_free import handle_utk_free
...
from Extractor.core.mongo.plans_db import check_premium
import datetime
...
custom_button = [[
                  ...
                  InlineKeyboardButton("📖 Uᴛᴋᴀʀsʜ", callback_data="utkarsh_free")
...
async def handle_callback(client, query):
    ...
    #Selection Way Handler
    elif query.data == "selection_way":
        await selection_way_handler(app, query.message)

    elif query.data == "noop":
        await query.answer()  # silently acknowledge

    #Utkarsh Free Handler
   # elif query.data == "utkarsh_free":
    #    await handle_utk_free(app, query.message)
        ...
        if user_id != OWNER_ID:
            ...
            if not premium or premium.get("expire_date", datetime.datetime.now()) < datetime.datetime.now():
                await query.answer("❌ Premium Feature!", show_alert=True)
                await query.message.reply_text(
                    "⚠️ **This feature is only for Owner/Premium Users.**\n\n"
                    "Contact owner to upgrade your plan.",
                    parse_mode=ParseMode.MARKDOWN
                )
                return

        # ✅ Allowed? Toh handler call karo
        await handle_utk_free(app, query.message)
